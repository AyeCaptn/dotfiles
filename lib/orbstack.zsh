...
source "$DOTFILES/lib/orbstack_context.zsh"
...
spaceship_orbstack() {
  ...
  local orbstack_context="$(spaceship_orbstack_context)"
  local orbstack_context_section="$(spaceship::section::render $orbstack_context)"
  ...
  [[ -n "$is_docker_project" || -f /.dockerenv || -n "$orbstack_context" ]] || return
  ...
  spaceship::section \
    ...
    "v${docker_version}${orbstack_context_section}"
}